...
function getParkingAround(){
	...
	var distance = 2000;
	var url = "http://api2.juheapi.com/park/query?key=47116fc8e7f748047f531cfd1f14471f&lx=1&skip=0&limit=20&wlyc=false&distance="+distance;
...
function searchParkingPlaces(){
	...
	if(!value){return;}
	var url = "http://api2.juheapi.com/park/query?key=47116fc8e7f748047f531cfd1f14471f&lx=1&skip=0&limit=20&wlyc=false&kw="+value;
	/*
	var latlng;
	if(window.localStorage.getItem("latitude")){
		latlng = {"lat": window.localStorage.getItem("latitude"), "lng": window.localStorage.getItem("longitude")};
...
function searchFootball(){
	...
	if(radio.checked){
		var url = "http://op.juhe.cn/onebox/football/league?key=dec3433ddcd0951da343a82a17b3d271&league="+text;
		checkFootballViews(url);
	}
	radio = $("#football_team")[0];
	if(radio.checked){
		url = "http://op.juhe.cn/onebox/football/team?key=dec3433ddcd0951da343a82a17b3d271&team="+text;
